...
{
    float f = f1 - f2;
    if ((f <= CLOSE) && (f >= -CLOSE)) return true;
    return false;
}
...
void initSparse()
{
    ...
    vals = (float*) malloc((n+8)*(n+8)*sizeof(float));
    printf("rows at 0x%x\n", &rows[0]);
    printf("Cols at 0x%x\n", &cols[0]);
    printf("vals at 0x%x\n", &vals[0]);
...
void genDenseVector(void)
{
    ...
    generateAsm("vadd_vv_v11_v10_v13_vm_macro", VFADD_VV(V11,V10,V13,VM));
    generateAsm("vmul_vv_v11_v10_v13_vm_macro", VFMUL_VV(V11,V10,V13,VM));
    ...
    generateAsm("vredsum_vs_v28_v28_v11_macro", VFREDSUM_VS(V28, V28, V11, VM));
    ...
    generateAsm("reset_v28_r0_macro", VFMV_VX(V28,R0));
}
...
int main(int argc, char ** argv)
{
    ...
    if (matrix_type == DENSE) {
        ...
        v = (float*)malloc((n+8)*sizeof(float));
        y = (float*)malloc((n+8)*sizeof(float));
        for (int i=0;i<n+8;i++) v[i] = 0;
        for (int i=0;i<n+8;i++) y[i] = 0;
        for (int i=0;i<n*n;i++) fscanf(fp,"%f\n", &m[i]);
        for (int i=0;i<n;i++) fscanf(fp,"%f\n", &v[i]);
    }
        ...
        while (fgets(&line[0], 1024, fp))
        {
            ...
            if (matrix_sz_found) {
                ...
                m = (float*)malloc(num_r*num_c*sizeof(float));
                v = (float*)malloc((num_c+8)*sizeof(float));
                y = (float*)malloc((num_c+8)*sizeof(float));
...
#define COMPARE (n-1)
    ...
    // check contents of y.
    printf("Last expected: %f\n", last_op);
    printf("Last output: %f\n", y[COMPARE]);
    if (!close(last_op, y[COMPARE])) printf("Failed!\n");
    else printf("Passed!\n");
